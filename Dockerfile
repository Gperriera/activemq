...
RUN mkdir -p /etc/gnutls/ && \
    ...
    aria2c --max-connection-per-server=4 --min-split-size=1M \
        https://archive.apache.org/dist/activemq/${activemqversion}/apache-activemq-${activemqversion}-bin.tar.gz && \
    tar -xzf apache-activemq-${activemqversion}-bin.tar.gz && \
    rm apache-activemq-${activemqversion}-bin.tar.gz
RUN mv apache-activemq-${activemqversion} apache-activemq
